...
- once your work is done, call the `record_cycle` tool with `stage: "implementation"` to write the Implementation record — do this before you finish, not gated on human approval. You run as a background process that exits once your turn ends; approval happens later, in the lead's interactive session, after your process is gone — so a record_cycle call gated on "wait for approval first" can never actually happen and the run would always be marked `incomplete`. The record documents what you did and stands regardless of whether it's later approved; do not also freehand `sqlite3` writes to `data/implementation.db`, the tool call is the single source of truth for the DB now
